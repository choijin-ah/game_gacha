...
Unity 상단의 `Starfall` 메뉴에서 주요 게임 데이터를 관리할 수 있습니다.

- `Starfall > Character Database`: 캐릭터 데이터 관리
- `Starfall > Stage Database`: 스테이지 데이터 관리
- `Starfall > Gacha Configuration`: 모집 확률과 비용 관리
- `Starfall > Story > Visual Novel Editor`: 에피소드와 대사 편집
- `Starfall > Story > Import Excel`: 스토리 시트 가져오기
- 각 `Rebuild ... Scene` 메뉴: 대응하는 씬 재생성

## 검증

Unity Editor 메뉴에서 핵심 로직 진단을 실행할 수 있습니다.

- `Starfall > Battle > Run Core Smoke Test`
- `Starfall > Meta > Run Core Diagnostics`
